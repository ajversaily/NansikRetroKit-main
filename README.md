...
Netlify, with checkout handled by an embedded Stripe payment form.
...
│                                  — the pages themselves (must stay at the project
│                                    root so Netlify's clean URLs work, e.g. /shop)
├── css/styles.css                — all site styling
├── js/
│   ├── store.js                  — product catalog (display), cart storage (localStorage)
│   ├── main.js                   — page behavior: product page, cart page, shop quick-view
...
Pages are reachable without `.html` (e.g. `/shop`, `/about`, `/cart`) — Netlify does this
automatically for static pages. Product pages use `/product/<id>` (e.g.
`/product/maradona-jersey`), which is rewritten to `product.html?id=...` via the redirect
rule in `netlify.toml`. `js/store.js`'s `productUrl()` is what generates these links.
...
1. `cart.html` calls `/api/create-checkout-session`, a Netlify Function
   (`netlify/functions/create-checkout-session.js`) that prices the cart using the
   server-side `lib/catalog.js` (never trusting a price from the browser) and asks Stripe
   to create an embedded Checkout Session.
...
Secrets never go in the repo — `STRIPE_SECRET_KEY` lives only as a Netlify environment
variable.
...
2. Import this repo into Netlify (or drag-and-drop deploy). Netlify auto-detects
   `netlify.toml`, which points it at the `netlify/functions` folder.
3. In Netlify → Site configuration → Environment variables, add `STRIPE_SECRET_KEY`.
...
For local testing, install the Netlify CLI and run `netlify dev` — it serves the site and
...
`netlify/functions/create-checkout-session.js`). The server price is the one that takes
the money — if they ever disagree, the customer is charged whatever `lib/catalog.js` says,
not what the page displays.
